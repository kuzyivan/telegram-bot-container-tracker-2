...
async def get_time(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    if not query or not query.data or not context.user_data or not query.from_user:
        return ConversationHandler.END
    ...
    context.user_data['selected_emails'] = set() # Используем set для хранения ID выбранных email
...
async def get_emails(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Обновляем клавиатуру, отмечая выбранные email
    user_emails = await get_user_emails(query.from_user.id)
    keyboard = []
    if user_emails:
        for email in user_emails:
...
async def get_name(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        ...
        f"Название: *{ud['name']}*\n"
        f"Контейнеры: `{', '.join(ud['containers'])}`\n"
        f"Время отчета: {ud['notify_time'].strftime('%H:%M')}\n"
    )
    
    email_ids = list(ud.get('selected_emails', []))
    if email_ids:
        # Нужен дополнительный запрос, чтобы получить тексты email по ID
...
async def confirm_creation(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(f"✅ Новая подписка '{ud['name']}' успешно создана!")
    context.user_data.clear()
    return ConversationHandler.END
