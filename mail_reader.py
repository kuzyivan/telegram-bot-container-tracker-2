...
async def find_container(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not rows:
        ...
        for row in rows:
            km_left = row[8]
            forecast_days = ceil(km_left / 600) if km_left > 0 else 0
            forecast_text = f"\n📅 Прогноз прибытия: через {forecast_days} дней" if forecast_days > 0 else ""
            message = (f"\U0001F69A Контейнер: {row[1]}\n"
                       ...
                       f"Дата/время: {row[6]}\n"
                       f"Накладная: {row[7]}\n"
                       f"Осталось км: {km_left}" + forecast_text)
