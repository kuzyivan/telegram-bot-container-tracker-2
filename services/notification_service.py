...
class NotificationService:
    ...
    async def _process_single_subscription(self, subscription):
        ...
        if not report_data_from_db:
            try:
                ...
                logger.error(f"Не удалось уведомить пользователя {user_id} об отсутствии данных: {e}", exc_info=True)
            return
        ...
        for db_row in report_data_from_db:
            ...
            # ❗️ Новое: Защитная проверка на наличие всех необходимых данных
            if db_row.from_station and db_row.to_station and db_row.current_station:
                try:
                    ...
                    logger.error(f"Ошибка при пересчете расстояния для ктк {db_row.container_number}: {e}", exc_info=True)
    ...
    # ... (методы _send_telegram_report_with_retry и _send_email_report остаются без изменений)
