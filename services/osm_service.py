...
logger.info("<<<<< ЗАГРУЖЕНА НОВАЯ ВЕРСИЯ OSM SERVICE v14.0 (финальный поиск) >>>>>")
...
async def _make_overpass_request(query: str) -> dict | None:
    ...
    async with httpx.AsyncClient(timeout=90.0) as client:
        try:
            ...
            # Логируем конкретную ошибку в вызывающих функциях
            return None
...
async def fetch_station_coords(station_name_for_search: str, original_station_name: str) -> dict | None:
    ...
    # Ищем все возможные типы ж/д остановок
    ...
    # Эвристика для выбора лучшего результата: сначала ищем 'station', потом 'halt' и т.д.
...
async def fetch_route_distance(from_station: str, to_station: str) -> int | None:
    ...
    Поиск полных маршрутов в OSM ненадёжен.
    """
    return None