...
from sqlalchemy import select, func, desc, update, and_ # <--- and_ добавлен
...
from models import User, UserRequest, Train, Company, UserRole, ScheduledTrain # <--- ScheduledTrain добавлен
...
# ... (Вспомогательные функции KPI и Clients Stats остаются без изменений, они используются в dashboard) ...
...
# ==========================================
# === 📅 КАЛЕНДАРЬ ПЛАНИРОВАНИЯ (NEW API) ===
# ==========================================
...
async def get_schedule_events(
    ...
    # FullCalendar присылает даты как '2023-12-01T00:00:00'
...
# 4. API: Перемещение события (Drag-and-Drop)
...
async def dashboard(
    ...
    return templates.TemplateResponse("dashboard.html", {
        "request": request, "user": current_user, **kpi_data, **clients_data,
        "chart_activity_labels": json.dumps(chart_activity_labels), "chart_activity_values": json.dumps(chart_activity_values), "feed_data": feed_data
    })
...
@router.get("/schedule") # Старый роут, оставим для совместимости пока
async def train_schedule_old(request: Request, db: AsyncSession = Depends(get_db), user: User = Depends(admin_required)):
    # Просто редиректим на новый планировщик, или оставляем старую таблицу
    # Лучше редирект:
    return RedirectResponse("/admin/schedule_planner")
